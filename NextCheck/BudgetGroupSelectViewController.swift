//
//  BudgetGroupSelectViewController.swift
//  CheckNow
//
//  Created by Khari Moore on 3/21/18.
...
class BudgetGroupSelectViewController: UITableViewController {
    static let ui_log = OSLog(subsystem: "com.example.NextCheck", category: "UI")
    var budgetGroups = [BudgetGroup]()
    var delegate: BudgetGroupSelectPopHandler?
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        do {
            try delegate?.AddBudgetGroup(budgetGroup: budgetGroup)
             navigationController?.popViewController(animated: true)
        } catch  {
            self.showAlertOK("Error", "Error occured adding budget group")
            os_log("Error occured adding budget group: %@", log: BudgetGroupViewController.ui_log, type: .error,error.localizedDescription)
        }
